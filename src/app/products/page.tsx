...
export const Route = createFileRoute("/products")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "MyPageSEO Product Ecosystem" },
      {
        property: "og:description",
        content: "Six integrated products built for businesses that live and die by local search.",
      },
    ...
    <div className="bg-background">
        ...
        <div className="container-page pt-24 pb-16 md:pt-28 md:pb-20 text-center">
          ...
          <div className="mt-14 relative mx-auto max-w-4xl">
            ...
            <div className="grid gap-4 sm:grid-cols-3 md:grid-cols-6">
              {products.map((p) => (
                <div key={p.slug} className="rounded-2xl border border-border/70 bg-background p-4 shadow-card transition-all hover:-translate-y-1 hover:shadow-lift">
                  <span className={`grid h-10 w-10 mx-auto place-items-center rounded-xl ${p.accent === "accent" ? "bg-accent/10 text-accent" : "bg-primary/5 text-primary"}`}>
                    <p.icon className="h-5 w-5" strokeWidth={1.75} />
                  </span>
                  <p className="mt-3 text-[11px] font-semibold text-foreground">{p.name}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="container-page py-24 space-y-24">
        {products.map((p, i) => {
          const reverse = i % 2 === 1;
          return (
            <div key={p.slug} className={`grid gap-12 lg:grid-cols-2 items-center ${reverse ? "lg:[&>*:first-child]:order-2" : ""}`}>
              <div>
                <div className="flex items-center gap-3">
                  <span className={`grid h-12 w-12 place-items-center rounded-2xl ${p.accent === "accent" ? "bg-accent/10 text-accent" : "bg-primary/5 text-primary"}`}>
                ...
                <h2 className="mt-5 text-3xl md:text-4xl font-semibold tracking-tight text-foreground">{p.name}</h2>
                <p className="mt-3 text-lg text-muted-foreground">{p.tagline}</p>
                <p className="mt-5 text-base text-foreground/80 leading-relaxed max-w-lg">{p.intro}</p>
                <ul className="mt-6 space-y-2">
                  {p.features.slice(0, 3).map((f) => (
                    <li key={f.title} className="flex items-start gap-2 text-sm text-foreground/80">
                      ...
                      <span><span className="font-semibold text-foreground">{f.title}.</span> {f.copy}</span>
                    </li>
                  ))}
                </ul>
                <Link to="/products/$slug" params={{ slug: p.slug }} className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-primary hover:text-accent transition-colors">
                  Explore {p.name} <ArrowRight className="h-4 w-4" />
                </Link>
              </div>

              <ProductMockup slug={p.slug} />
            </div>
          );
        })}
      </section>
      ...
      <section className="container-page py-24">
        <div className="relative overflow-hidden rounded-3xl px-8 py-20 md:px-16 text-center" style={{ background: "linear-gradient(135deg, var(--color-primary) 0%, color-mix(in oklab, var(--color-primary) 80%, var(--color-accent) 20%) 100%)" }}>
...
function ProductMockup({ slug }: { slug: string }) {
  switch (slug) {
    case "mypageseo":
